(function () {
...
})()(
  {
    ...
    5: [
      function (require, module, exports) {
        ...
        Correlation.pearson = function (x, y) {
          ...
          result.t =
            result.r * Math.sqrt((n - 2) / (1 - Math.pow(result.r, 2)));
          result.df = n - 2;
    ...
    6: [
      function (require, module, exports) {
        ...
        Kolmogorov.prototype._di = function (x) {
          ...
          do {
            term = Math.exp(
              (-Math.pow(2 * k - 1, 2) * Math.pow(Math.PI, 2)) /
                (8 * Math.pow(x, 2))
            );
            sum = sum + term;
            k++;
    ...
    9: [
      function (require, module, exports) {
        ...
        Misc.gamma = function (n) {
          ...
          return (
            Math.sqrt(2 * Math.PI) * Math.pow(t, n + 0.5) * Math.exp(-t) * a
          );
        };
    ...
    11: [
      function (require, module, exports) {
        ...
        Normality.shapiroWilk = function (x) {
          ...
          if (n > 5) {
            ...
            phi =
              (md - 2 * Math.pow(m.elements[n - 1], 2)) /
              (1 - 2 * Math.pow(an, 2));
          }
          ...
          if (n < 12) {
            ...
            mu =
              ...
              1.5851;
            sigma = Math.exp(
              0.0030302 * Math.pow(u, 2) - 0.082676 * u - 0.4803
            );
          }
      ...
      { "./distributions": 6, "./matrix": 8, "./vector": 16 },
    ],
    12: [
      function (require, module, exports) {
    ...
    13: [
      function (require, module, exports) {
        ...
        Power.sampleSize = function (a, power, sd, effect) {
          ...
          return (
            (2 *
              Math.pow(n.inverse(1 - a / 2) + n.inverse(power), 2) *
              Math.pow(sd, 2)) /
            Math.pow(effect, 2)
          );
        };
    ...
    14: [
      function (require, module, exports) {
        ...
        Regression.linear = function (x, y) {
          ...
          var ssr = y
            .add(x.multiply(result.slope).add(result.intercept).multiply(-1))
            .pow(2)
            .sum();
          ...
          result.intercept_se =
            (result.rse / Math.sqrt(ssxx) / Math.sqrt(result.n)) *
            Math.sqrt(x.pow(2).sum());
          result.intercept_t = result.intercept / result.intercept_se;
          result.intercept_p =
            2 * (1 - tdistr.distr(Math.abs(result.intercept_t)));
    ...
    15: [
      function (require, module, exports) {
        ...
        StudentT._twosample = function (first, second) {
          var result = {};
          result.se = Math.sqrt(
            first.variance() / first.length() +
              second.variance() / second.length()
          );
        ...
        StudentT._onesample = function (sample, mu) {
          ...
          result.mu = mu;
          result.se =
            Math.sqrt(result.sample.variance()) /
            Math.sqrt(result.sample.length());
    ...
    16: [
      function (require, module, exports) {
        ...
        Vector.prototype.kurtosis = function () {
          return (
            this.res().pow(4).mean() / Math.pow(this.res().pow(2).mean(), 2)
          );
        };

        Vector.prototype.skewness = function () {
          return (
            this.res().pow(3).mean() / Math.pow(this.res().pow(2).mean(), 3 / 2)
          );
        };
